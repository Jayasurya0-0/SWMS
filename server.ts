...
import { getDb, saveDb, createJob, updateJob, getJob, getCachedKpis, getCachedProductivity, initializeFirestoreDb, resetDbInMemory } from './src/data/dbService.js';
...
app.post('/api/system/settings', (req, res) => {
  const db = getDb();
  const { systemDate, overallTarget, overallActual, theme, currentUser, lockedLines, currentGarmentId } = req.body;
  ...
  if (currentUser !== undefined) db.currentUser = currentUser;
...
app.post('/api/employee-assignments', (req, res) => {
  ...
  const nextAss: any = {
    ...
    assignedBy: db.currentUser?.username || 'Admin',
...
app.post('/api/line-style-assignments/change', (req, res) => {
  const db = getDb();
  const { lineNumber, garmentStyleId, effectiveDate, effectiveTime, reason, remarks, changedBy } = req.body;
  ...
  const historyRecord = {
    ...
    changedBy: changedBy || db.currentUser?.username || 'Industrial Engineer',
...
app.post('/api/export-logs', (req, res) => {
  ...
  const newLog = {
    ...
    username: username || db.currentUser?.username || 'Unknown',
    role: role || db.currentUser?.role || 'Guest',
    reportType,
    exportFormat,
    dateRange,
    timestamp: new Date().toISOString(),
...
app.post('/api/auth/login', (req, res) => {
  ...
  writeAuditLog(db, user.id, 'User Login Success', 'System', `Platform access granted. Role: ${user.role}`);
  saveDb();
...
app.post('/api/auth/logout', (req, res) => {
  const db = getDb();
  if (db.currentUser) {
    writeAuditLog(db, db.currentUser.id, 'User Logout', 'System', 'Session closed successfully');
  }
  db.currentUser = null;
  saveDb();
  res.json({ success: true });
...
app.post('/api/auth/change-password-user', (req, res) => {
  ...
  if (userInList) {
    ...
    caller.passwordHash = userInList.passwordHash;
  }
...
app.post('/api/auth/update-profile-user', (req, res) => {
  ...
  if (userInList) {
    ...
    // Synced
    caller.email = userInList.email;
    caller.employeeName = userInList.employeeName;
    caller.avatarUrl = userInList.avatarUrl;
  }
