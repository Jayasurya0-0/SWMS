...
const LoadingStepItem: React.FC<LoadingStepItemProps> = ({ label, delay, activeColorClass, theme }) => {
  ...
  return (
    <div className="flex items-center space-x-2.5 text-[11.5px] font-sans">
      ...
      <span className={`transition ${
          ...
          ? theme === 'dark' ? 'text-slate-300' : 'text-slate-705 font-medium' 
          : 'text-slate-400 dark:text-slate-505 italic'
      } flex-1`}>
        {label}
      </span>
      {done && (
        <span className="text-[9.5px] font-mono font-bold text-emerald-500 animate-fade-in self-center shrink-0">COMPLETE</span>
      )}
    </div>
  );
};
...
interface PortalSwitchLoaderProps {
  targetPortal: 'HR' | 'IE' | null;
  theme: 'light' | 'dark';
}

const PortalSwitchLoader: React.FC<PortalSwitchLoaderProps> = ({ targetPortal, theme }) => {
  const isIe = targetPortal === 'IE';
  ...
  return (
    <motion.div 
      ...
      exit={{ opacity: 0 }}
      className={`min-h-[60vh] flex flex-col items-center justify-center p-8 rounded-2xl border transition-all ${
        theme === 'dark' 
          ? 'bg-[#111827]/80 border-slate-800' 
          : 'bg-[#FFFFFF] border-slate-200 shadow-xl shadow-slate-100/40'
      }`}
    >
      <div className="relative flex flex-col items-center w-full max-w-sm text-center">
        {/* Outer ambient pulsing halo */}
        <div className={`absolute -inset-10 rounded-full blur-3xl opacity-20 pointer-events-none animate-pulse ${
          isIe ? 'bg-indigo-500' : 'bg-blue-500'
        }`} />

        {/* Brand logo container */}
        <div className={`w-14 h-14 rounded-2xl flex items-center justify-center mb-5 shadow-sm transition-all ${
          isIe 
            ? 'bg-gradient-to-tr from-indigo-550 to-indigo-650 text-white' 
            : 'bg-gradient-to-tr from-blue-500 to-blue-650 text-white'
        }`}>
        ...
        <div className="relative w-12 h-12 mb-5">
          ...
          <motion.div 
            ...
            className={`absolute inset-0 rounded-full border-4 border-t-transparent ${
              isIe ? 'border-indigo-500' : 'border-blue-600'
            }`}
          />
        </div>

        {/* Text Area */}
        <h3 className={`text-base font-bold tracking-tight transition ${
          theme === 'dark' ? 'text-slate-100' : 'text-slate-800'
        }`}>
          {isIe ? 'Switching to Industrial Engineering Portal' : 'Switching to HR & Compliance Portal'}
        </h3>
        
        <p className={`text-xs mt-2 font-mono transition duration-200 ${
          isIe ? 'text-indigo-500 font-bold' : 'text-blue-600 font-bold'
        }`}>
          {isIe ? 'Loading IE Core Engine...' : 'Syncing Human Personnel Nodes...'}
        </p>

        {/* High-fidelity procedural checkpoints */}
        <div className="mt-6 w-full max-w-xs text-left space-y-2 border-t border-slate-100 dark:border-slate-800/60 pt-4">
          <LoadingStepItem 
            label={isIe ? 'Establishing secure database links' : 'Pulling active personnel master rows'} 
            delay={100} 
            activeColorClass={isIe ? 'bg-indigo-500' : 'bg-blue-605'} 
            theme={theme} 
          />
          <LoadingStepItem 
            label={isIe ? 'Parsing operator skill proficiency metrics' : 'Reading plant gate biometric scan registers'} 
            delay={250} 
            activeColorClass={isIe ? 'bg-indigo-500' : 'bg-blue-605'} 
            theme={theme} 
          />
          <LoadingStepItem 
            label={isIe ? 'Pre-rendering factory line layout config' : 'Compiling historic absenteeism registers'} 
            delay={500} 
            activeColorClass={isIe ? 'bg-indigo-500' : 'bg-blue-605'} 
            theme={theme} 
          />
...
function DashboardPortal() {
  ...
  const [switchingTo, setSwitchingTo] = useState<'HR' | 'IE' | null>(null);

  const handlePortalSwitch = (portal: 'HR' | 'IE') => {
    if (portal === resolvedPortal) return;
    setIsSwitchingPortal(true);
    setSwitchingTo(portal);
  ...
  const navigationItems = React.useMemo(() => {
    if (resolvedPortal === 'HR') {
      const allHrProducts = [
        ...
        { id: 'import-export', label: 'Data Upload/Export Center', icon: FileSpreadsheet },
      ...
      <main className="flex-1 flex flex-col h-screen min-w-0 overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto p-4 md:p-6 custom-scrollbar bg-slate-50 dark:bg-[#0B0F19]">
          <AnimatePresence mode="popLayout">
            {isSwitchingPortal ? (
              <PortalSwitchLoader key="switching" targetPortal={switchingTo} theme={theme} />
            ) : (
              <motion.div
                key={activeTab}
