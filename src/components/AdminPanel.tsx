...
export const AdminPanel: React.FC = () => {
  const { 
    ...
    sessions, fetchSessions, terminateSession
  } = useAppState();

  // Active Main Tabs: 'database' | 'users' | 'audit_logs' | 'query' | 'sessions'
  const [activeTab, setActiveTab] = useState<'database' | 'users' | 'audit_logs' | 'query' | 'sessions'>('database');
  ...
  // Load audit logs and sessions on activation and layout changes
  useEffect(() => {
    fetchAuditLogs();
          ...
          <AnimatePresence mode="wait">
            ...
            {activeTab === 'sessions' && (
              <motion.div 
                ...
                animate={{ opacity: 1, y: 0 }}
                exit={{ opacity: 0, y: -4 }}
                className="space-y-4 animate-fade-in w-full col-span-1"
              >
                {/* Stats row */}
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div className="bg-white dark:bg-[#121929] border border-slate-200 dark:border-slate-800 rounded-xl p-4 shadow-sm">
                    <div className="text-xs font-semibold text-slate-400">Total Authenticated Sessions</div>
                    <div className="mt-1 flex items-baseline justify-between">
                      <span className="text-2xl font-mono font-bold text-slate-800 dark:text-neutral-150">
                        {sessions?.length || 0}
                      </span>
                      <span className="text-[10px] text-slate-400">Persistent Ledger</span>
                    </div>
                  </div>

                  <div className="bg-white dark:bg-[#121929] border border-slate-200 dark:border-slate-800 rounded-xl p-4 shadow-sm">
                    <div className="text-xs font-semibold text-slate-400">Active Handsets & Browsers</div>
                    <div className="mt-1 flex items-baseline justify-between">
                  ...
                  <div className="bg-white dark:bg-[#121929] border border-slate-200 dark:border-slate-800 rounded-xl p-4 shadow-sm">
                    <div className="text-xs font-semibold text-slate-400 font-sans">Terminated sessions</div>
                    <div className="mt-1 flex items-baseline justify-between">
                      <span className="text-2xl font-mono font-bold text-slate-400 font-sans">
                        {sessions?.filter(s => s.status !== 'Active').length || 0}
                      </span>
                      <span className="text-[10px] text-slate-400 font-sans">Security Eviction List</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-white dark:bg-[#121929] border border-slate-200 dark:border-slate-800 rounded-xl p-5 shadow-sm space-y-4">
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                    <div>
                      <h3 className="font-display font-semibold text-slate-800 dark:text-neutral-100 text-sm">
                        Device-Specific Security Session Registry
                      </h3>
                      <p className="text-xs text-slate-400">
                        Isolated bearer tokens associated with distinct browser sessions. Administrators can revoke specific credentials in real-time.
                      </p>
                    </div>
                    <button
                      type="button"
                      onClick={() => fetchSessions()}
                      className="p-1 px-2.5 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-750 text-slate-700 dark:text-slate-300 rounded-lg text-[11px] font-sans transition flex items-center space-x-1"
                    >
                  ...
                  <div className="overflow-x-auto border border-slate-150 dark:border-slate-850 rounded-xl">
                    <table className="w-full text-xs text-left border-collapse font-sans">
                      <thead>
                        <tr className="bg-slate-50 dark:bg-slate-900/50 border-b border-slate-200 dark:border-slate-800 text-slate-500 font-semibold select-none">
                          <th className="px-3 py-2.5">User Identity</th>
                          <th className="px-3 py-2.5">Platform & Browser</th>
                          <th className="px-3 py-2.5">Network Address</th>
                          <th className="px-3 py-2.5">Temporal Ledger (Login & Activity)</th>
                          <th className="px-3 py-2.5">State Status</th>
                          <th className="px-3 py-2.5 text-right">Access Controls</th>
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-slate-150 dark:divide-slate-850">
                        {(sessions || []).map(sess => {
                          const isCurrent = typeof document !== 'undefined' && document.cookie.includes(`sid=${sess.id}`);
                          return (
                            <tr key={sess.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-900/10 transition">
                              <td className="px-3 py-3">
                                <div className="font-semibold text-slate-800 dark:text-slate-250 flex items-center space-x-1.5">
                                  <span>{sess.employeeName || sess.username}</span>
                                  {isCurrent && (
                                    <span className="text-[9px] bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400 px-1.5 py-0.5 rounded-full font-sans font-bold">
                                      CURRENT
                                    </span>
                                  )}
                                </div>
                                <div className="text-[10px] text-slate-400 flex items-center space-x-1.5 mt-0.5">
                                  <span className="font-mono">{sess.username}</span>
                                  <span className="text-slate-300 dark:text-slate-700">|</span>
                                  <span className="bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 px-1.5 py-0.2 rounded text-[9px] select-none uppercase font-mono font-bold">
                                    {sess.role}
                                  </span>
                                </div>
                              </td>
                              ...
                              <td className="px-3 py-3">
                                <div className="text-slate-700 dark:text-slate-350 font-medium font-sans">
                                  {sess.deviceInfo || 'Desktop'}
                                </div>
                                <div className="text-[10px] text-slate-400 mt-0.5 font-mono">
                                  {sess.browserInfo || 'Chrome'}
                                </div>
                              </td>
                              ...
                              <td className="px-3 py-3 font-mono text-[10px] space-y-0.5 text-slate-600 dark:text-slate-400">
                                <div className="flex items-center space-x-1">
                                  <span className="text-slate-400 text-[9px] w-9 font-sans">Login:</span>
                                  <span>{sess.loginTime ? new Date(sess.loginTime).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit', second:'2-digit'}) : 'Prior session'}</span>
                                </div>
                                <div className="flex items-center space-x-1">
                                  <span className="text-slate-400 text-[9px] w-9 font-sans">Active:</span>
                                  <span>{sess.lastActivityTime ? new Date(sess.lastActivityTime).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit', second:'2-digit'}) : 'Unrecorded'}</span>
                                </div>
                              </td>
                              ...
                              <td className="px-3 py-3">
                                <div className="flex items-center space-x-2">
                                  <span className={`w-2 h-2 rounded-full ${
                                    sess.status === 'Active' ? 'bg-emerald-500 animate-pulse' : 'bg-slate-300'
                                  }`}></span>
                                  <span className={`text-[11px] font-semibold font-sans ${
                                    sess.status === 'Active' ? 'text-emerald-600 dark:text-emerald-450' : 'text-slate-400'
                                  }`}>
                                    {sess.status === 'Active' ? 'Live Authorization' : 'Revoked / Terminated'}
                                  </span>
                                </div>
                              </td>
                              ...
                              <td className="px-3 py-3 text-right">
                                {sess.status === 'Active' ? (
                                  <button
                                    type="button"
                                    onClick={async () => {
                                      if (confirm(`Are you absolutely sure you want to forcibly evict and terminate the security token for ${sess.username}?`)) {
                                    ...
                                    className="p-1 px-2 w-max text-[10px] bg-red-50 text-red-600 hover:bg-red-100 dark:bg-red-950/20 dark:text-red-400 dark:hover:bg-red-900/30 rounded-lg transition"
                                  >
                                    Terminate Session
                                  </button>
                                ) : (
                        ...
                        {(!sessions || sessions.length === 0) && (
                          <tr>
                            <td colSpan={6} className="py-8 text-center text-slate-400 font-sans">
                              No authenticated session nodes recorded under current cluster authority.
                            </td>
                          </tr>
                        )}
                      </tbody>
