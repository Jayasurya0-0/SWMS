...
import { 
  ...
  LineStyleAssignment, GarmentStyleHistory, AuditLogEntry
} from '../types';
import { 
...
// Read config dynamically from file to prevent compile issues and remain secure
...
export async function syncToFirestore(data: Schema): Promise<void> {
  ...
  try {
    ...
    if (!lastSyncedSettings || !isEquivalent(lastSyncedSettings, currentSettings)) {
      try {
    ...
    await Promise.all([
      syncCollectionAndCleanOrphans('employees', data.employees, (item) => item.id.toUpperCase()),
      syncCollectionAndCleanOrphans('attendance', data.attendance, (item) => item.id || `att_${item.employeeId}_${item.date}`),
      syncCollectionAndCleanOrphans('leaveRequests', data.leaveRequests, (item) => item.id),
      syncCollectionAndCleanOrphans('productionLines', data.productionLines, (item) => String(item.id)),
      syncCollectionAndCleanOrphans('notifications', data.notifications, (item) => item.id),
      syncCollectionAndCleanOrphans('dailyProductivity', data.dailyProductivity, (item) => item.id),
      syncCollectionAndCleanOrphans('lineAllocations', data.lineAllocations, (item) => item.employeeId.toUpperCase()),
      syncCollectionAndCleanOrphans('employeeAssignments', data.employeeAssignments, (item) => item.id || `asgn_${item.employeeId}_${item.assignmentDate}`),
      syncCollectionAndCleanOrphans('departments', data.departments, (item) => item.id),
      syncCollectionAndCleanOrphans('operations', data.operations, (item) => item.code),
      syncCollectionAndCleanOrphans('garmentStyles', data.garmentStyles, (item) => item.id),
      syncCollectionAndCleanOrphans('lineStyleAssignments', data.lineStyleAssignments || [], (item) => item.id),
      syncCollectionAndCleanOrphans('garmentStyleHistory', data.garmentStyleHistory || [], (item) => item.id),
      syncCollectionAndCleanOrphans('auditLogs', data.auditLogs || [], (item) => item.id),
      syncCollectionAndCleanOrphans('allUsers', data.allUsers || SYSTEM_USERS, (item) => item.id)
    ]);
  } catch (err) {
...
export function getDb(): Schema {
  ...
  if (!dbInMemory) {
    const baseEmp = INITIAL_EMPLOYEES.map(e => {
      ...
      const isEligible = dept === 'sewing' || dept === 'floater';
